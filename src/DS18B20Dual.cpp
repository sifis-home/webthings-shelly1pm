...
/*------------------------------------------------------------------------------------------*/
#define USE_DS18B20_INTERNAL_PULLUP (true)

uint8_t OneWireReset(void) {
    uint8_t retries = 125;

    if (!ds18x20_dual_mode) {
        pinMode(ds18x20_pin, USE_DS18B20_INTERNAL_PULLUP ? INPUT_PULLUP
...
//**************************************************
    ...
    pinMode(ds18x20_pin, USE_DS18B20_INTERNAL_PULLUP ? INPUT_PULLUP : INPUT);

    // LogUDP("OneWireResetSearch start");
    OneWireResetSearch();
    // LogUDP("OneWireResetSearch end");

    ds18x20_sensors = 0;
    while (ds18x20_sensors < DS18X20_MAX_SENSORS) {
        // LogUDP("Sensor Iteration");
    ...
    // LogUDP("DS18B20 Discovered: " + String(ds18x20_sensors));


}

void Ds18x20Convert(void) {
    // LogUDP("Ds18x20Convert");
        ...
        if (OneWireCrc8(data)) {
            switch (ds18x20_sensor[index].address[0]) {
                case DS18S20_CHIPID: {
                    ...
                    ds18x20_sensor[index].temperature = (temp9 - 0.25) + ((16.0 - data[6]) / 16.0);
                ...
                case DS18B20_CHIPID: {
                    ...
                    ds18x20_sensor[index].temperature = sign * temp12 * 0.0625;  // Divide by 16
                    ds18x20_sensor[index].valid = SENSOR_MAX_MISS;
                    return true;
                }
                case MAX31850_CHIPID: {
                    int16_t temp14 = (data[1] << 8) + (data[0] & 0xFC);
                    ds18x20_sensor[index].temperature = temp14 * 0.0625;  // Divide by 16
                    ds18x20_sensor[index].valid = SENSOR_MAX_MISS;
                    return true;
                }
            }
    ...
    if (uptime & 1
        ...
        for (uint32_t i = 0; i < ds18x20_sensors; i++) {
            ...
            if (!Ds18x20Read(i)) {   // Read temperature
                // LogUDP("Error while reading ds18b20");
...
void Ds18x20Show() {
    ...
    for (uint32_t i = 0; i < ds18x20_sensors; i++) {
        ...
        if (ds18x20_sensor[index].valid) {   // Check for valid temperature
            auto temp = ds18x20_sensor[index].temperature;
            Shelly1PMManager::getInstance().setTemperature(temp);
            // LogUDP("Read DS18B20" + String(temp));
        }
    }
